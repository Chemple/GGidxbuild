...
std::vector<std::vector<uint32_t>> MatchNN(
  ...
  SPDLOG_INFO("MatchNN: Set OpenMP threads to {} (was {})", 
              omp_get_num_threads(), original_threads);

  auto start_time = std::chrono::high_resolution_clock::now();
  SPDLOG_INFO(
      "MatchNN: Starting with num_base={}, num_query={}, N_ctr={}, M_nn={}",
      num_base, num_query, N_ctr, M_nn);

  std::vector<uint32_t> match(num_query, num_base + 1);
  std::vector<uint32_t> frequency(num_base, 0);
  std::vector<bool> vis(num_base, false);

  SPDLOG_INFO("MatchNN: Computing initial matches and frequencies");
  for (uint32_t it_nq = 0; it_nq < num_query; ++it_nq) {
    ...
    for (uint32_t j = 0; j < N_ctr; j++) {
      ...
      if (nn >= num_base || ifmatch) break;  // 严格边界检查
      ++frequency[nn];
      if (vis[nn]) {
        continue;
      } else {
  ...
  SPDLOG_INFO("MatchNN: Sorting nodes by frequency");
  ...
  ep = order[0];
  SPDLOG_INFO("MatchNN: Selected entry point (ep) = {}", ep);
  ...
  SPDLOG_INFO(
      "MatchNN: Building initial graph with OpenMP parallelism (MAX_DEGREE={})",
      MAX_DEGREE);
...
#pragma omp parallel for schedule(dynamic, 200)
  for (uint32_t it_nq = 0; it_nq < num_query; ++it_nq) {
    ...
    for (uint32_t j = 0; j < N_ctr; j++) {
      ...
      if (base_id >= num_base) break;  // 更严格的边界检查
  ...
  SPDLOG_INFO("MatchNN: Merging and pruning final graph");
...
#pragma omp parallel for schedule(dynamic, 200)
  ...
  for (auto const& neighbors : match_graph) {
...
std::vector<std::vector<uint32_t>> FusionFinal(
  ...
  int original_threads = omp_get_max_threads();
  omp_set_num_threads(thread_limit);
  SPDLOG_INFO("FusionFinal: Set OpenMP threads to {} (was {})", 
              omp_get_num_threads(), original_threads);
              
  auto start_time = std::chrono::high_resolution_clock::now();
  SPDLOG_INFO(
      "FusionFinal: Starting with num_base={}, M_supply={}, M_link={}, "
      "M_final={}",
      num_base, M_supply, M_link, M_final);

  std::vector<std::vector<uint32_t>> final_graph_(num_base);

  size_t invalid_base_ids = 0;
  size_t invalid_link_ids = 0;

  SPDLOG_INFO("FusionFinal: Building final graph with OpenMP parallelism");
  auto before_parallel = std::chrono::high_resolution_clock::now();

#pragma omp parallel for schedule(dynamic, 100) \
    reduction(+ : invalid_base_ids, invalid_link_ids)
  for (uint32_t it_nb = 0; it_nb < num_base; ++it_nb) {
    ...
    for (uint32_t j = 0; j < M_supply; j++) {
      ...
      if (base_id >= num_base &&
          base_id != 0xFFFFFFFF) {  // 使用十六进制常数表示更清晰
        invalid_base_ids++;
      }
    ...
    for (uint32_t j = 0; j < M_link; j++) {
      ...
      if (base_id >= num_base && base_id != 0xFFFFFFFF) {
        invalid_link_ids++;
      }
      if (base_id >= num_base) continue;
  ...
  for (auto const& neighbors : final_graph_) {
    total_edges += neighbors.size();
    min_degree = std::min(min_degree, neighbors.size());
  ...
  if (invalid_base_ids > 0) {
    SPDLOG_WARN("FusionFinal: Found {} invalid base IDs in supply graph",
                invalid_base_ids);
  }
  if (invalid_link_ids > 0) {
    SPDLOG_WARN("FusionFinal: Found {} invalid base IDs in link graph",
                invalid_link_ids);
  }
  ...
  SPDLOG_INFO("FusionFinal: Completed in {:.2f} seconds",
              total_duration.count());
  SPDLOG_INFO(
      "FusionFinal: Graph statistics - Edges: {}, Avg degree: {:.2f}, Min "
      "degree: {}, Max degree: {}",
      total_edges, avg_degree, min_degree, max_degree);
...
void SaveGraph(std::vector<std::vector<uint32_t>> const& graph,
  ...
  auto start_time = std::chrono::high_resolution_clock::now();
  SPDLOG_INFO("SaveGraph: Saving graph to {}", file_path);

  std::ofstream out(file_path, std::ios::binary | std::ios::out);
  if (!out.is_open()) {
    SPDLOG_ERROR("SaveGraph: Failed to open file for writing: {}", file_path);
  ...
  double avg_degree = static_cast<double>(total_edges) / base_num;
  SPDLOG_INFO(
      "SaveGraph: Graph saved successfully with {} nodes, {} edges (avg "
      "degree: {:.2f})",
      base_num, total_edges, avg_degree);
  SPDLOG_INFO("SaveGraph: Completed in {:.2f} seconds", total_duration.count());
}

TEST(GpuConstructionTime, TestEnd2EndCPUGPU) {
  SPDLOG_INFO(
      "============== Starting End-to-End CPU+GPU Graph Construction Test "
      "==============");
  auto test_start_time = std::chrono::high_resolution_clock::now();

  cudaDeviceReset();
  SPDLOG_INFO("CUDA device reset completed");
  ...
  SPDLOG_INFO("Setting CPU thread limit to {} (max available: {})", 
              cpu_thread_limit, max_threads);

  SPDLOG_INFO("Configuration parameters:");
  SPDLOG_INFO("  - Base vectors: {}, Dimension: {}", base_num, dim);
  SPDLOG_INFO("  - GT degree: {}, Match degree: {}", gt_degree, match_degree);
  SPDLOG_INFO("  - First round: Grid={}, Block={}, Shared mem={}B",
              first_round_search_grid_size, first_round_search_block_size,
              shared_memory_size);
  SPDLOG_INFO("  - Second round: Grid={}, Block={}, Shared mem={}B",
              second_round_search_grid_size, second_round_search_block_size,
              second_shared_memory_size);
  SPDLOG_INFO("  - Final degree: {}", final_degree);
  ...
  // 读取数据
  SPDLOG_INFO("Loading input data from files");
  ...
  SPDLOG_INFO("  - Ground truth data: {} elements", h_gt_data.size());
  SPDLOG_INFO("  - Base vectors: {} elements", h_base_data.size());
  ...
  // 分配GPU内存
  SPDLOG_INFO("Allocating GPU memory");
  ...
  size_t total_gpu_memory =
      base_num * dim * sizeof(float) +           // d_base_data
      base_num * gt_degree * sizeof(uint32_t) +  // d_space_128_xx
      base_num * sizeof(pr_neighbor_list<uint32_t, reverse_edge_num,
                                         pruned_edge_num>) +  // d_space_128_yy
      base_num * top1_projection_degree *
          sizeof(uint32_t) +                                // d_top1_projection
      global_warp_num * hashtable_size * sizeof(uint32_t);  // d_hashtables

  SPDLOG_INFO("Total GPU memory used: {:.2f} GB",
              total_gpu_memory / (1024.0 * 1024.0 * 1024.0));

  // 开始计时
  SPDLOG_INFO("Starting GPU computation and timing");
  cudaEventRecord(start, compute_stream);

  // 1. 异步数据传输到GPU (在copy_stream上)
  SPDLOG_INFO("Transferring data to GPU");
  ...
  SPDLOG_INFO("Data transfer to GPU initiated");
 
  // 2. GPU计算第一阶段 - top1 projection
  SPDLOG_INFO("GPU: Waiting for data transfer before starting phase 1");
  cudaStreamWaitEvent(compute_stream, data_ready, 0);

  // GPU Kernels - 第一阶段 (top1 projection)
  SPDLOG_INFO("GPU: Starting phase 1 computation - top1 projection");
  constexpr uint32_t match_grid_size = 144;
  constexpr uint32_t match_block_size = 512;

  SPDLOG_INFO("GPU: Executing match_top1_kernel_v0");
  ...
  SPDLOG_INFO("GPU: Executing init_pr_lists");
  ...
  SPDLOG_INFO("GPU: Executing fusion_prune_reverse_kernel_v0");
  ...
  SPDLOG_INFO("GPU: Executing fusion_merge_sort_prune_kernel (phase 1)");
  ...
  SPDLOG_INFO(
      "GPU: Phase 1 computation completed, preparing to copy results to host");
  ...
  SPDLOG_INFO("GPU: Copying phase 1 projection data to host");
  ...
  SPDLOG_INFO("GPU: Phase 1 completed and signaled to CPU");

  // 3. 等待GPU第一阶段完成，然后CPU计算topnn projection
  SPDLOG_INFO("CPU: Waiting for GPU phase 1 (top1 projection) completion");
  auto gpu1_wait_start = std::chrono::high_resolution_clock::now();

  cudaError_t phase1_result = cudaEventSynchronize(gpu_phase1_done);
  if (phase1_result != cudaSuccess) {
    SPDLOG_ERROR("CPU: Error waiting for GPU phase 1: {}", 
                 cudaGetErrorString(phase1_result));
  } else {
    SPDLOG_INFO("CPU: Received top1 projection data from GPU");
  }
  ...
  // 验证top1 projection数据
  size_t valid_projection_count = 0;
  for (size_t i = 0; i < std::min(size_t(1000), h_projection.size()); i++) {
    if (h_projection[i] < base_num) valid_projection_count++;
  }
  SPDLOG_INFO("CPU: Top1 projection data validation - {} valid IDs in first 1000 entries",
              valid_projection_count);

  // CPU计算topnn projection (与GPU第二阶段并行)
  SPDLOG_INFO("CPU: Starting MatchNN computation for topnn projection");
  ...
  SPDLOG_INFO("GPU: Starting phase 2 computation - link operations");

  SPDLOG_INFO("GPU: Executing link_process_v0 (first round)");
  ...
  SPDLOG_INFO("GPU: Executing init_pr_lists (first round)");
  ...
  SPDLOG_INFO("GPU: Executing fusion_prune_reverse_kernel_v0 (first round)");
  ...
  SPDLOG_INFO("GPU: Executing fusion_merge_sort_prune_kernel (first round)");
  ...
  SPDLOG_INFO("GPU: Executing link_process_v0 (second round)");
  ...
  SPDLOG_INFO("GPU: Executing init_pr_lists (second round)");
  init_pr_lists<grid_size, block_size, base_num, match_degree,
  ...
  SPDLOG_INFO("GPU: Executing fusion_prune_reverse_kernel_v0 (second round)");
  ...
  SPDLOG_INFO("GPU: Executing fusion_merge_sort_prune_kernel (final)");
  fusion_merge_sort_prune_kernel<grid_size, block_size, base_num,
                                 second_round_pruned_edge_num,
  ...
  // 第二阶段完成，准备数据传输
  SPDLOG_INFO(
      "GPU: Phase 2 computation completed, preparing to copy results to host");
  ...
  // 5. 异步拷贝最终结果给CPU使用
  SPDLOG_INFO("GPU: Copying final search results to host");
  ...
  SPDLOG_INFO("GPU: Phase 2 completed and signaled to CPU");

  // 等待GPU第二阶段完成
  SPDLOG_INFO("CPU: Waiting for GPU phase 2 (link graph) completion");
  auto gpu2_wait_start = std::chrono::high_resolution_clock::now();

  cudaError_t phase2_result = cudaEventSynchronize(gpu_phase2_done);
  if (phase2_result != cudaSuccess) {
    SPDLOG_ERROR("CPU: Error waiting for GPU phase 2: {}", 
                 cudaGetErrorString(phase2_result));
  } else {
    SPDLOG_INFO("CPU: Received link graph data from GPU");
  }
  ...
  // 验证link graph数据
  size_t valid_search_merge_count = 0;
  for (size_t i = 0; i < std::min(size_t(1000), h_second_round_search_merge.size()); i++) {
    if (h_second_round_search_merge[i] < base_num) valid_search_merge_count++;
  }
  SPDLOG_INFO("CPU: Link graph data validation - {} valid IDs in first 1000 entries",
              valid_search_merge_count);
  
  SPDLOG_INFO(
      "CPU: Ready to fusion - top1 projection: {} entries, topnn projection: {} nodes, link graph: {} entries",
      h_projection.size(), topnn_projection_graph.size(), h_second_round_search_merge.size());

  // 6. 最终阶段 - 融合三个图
  SPDLOG_INFO("CPU: Starting FusionFinal computation");
  ...
  // 7. 保存最终图
  SPDLOG_INFO("CPU: Saving final graph");
  ...
  // 清理资源
  SPDLOG_INFO("Cleaning up CUDA resources");
  ...
  SPDLOG_INFO("Total test execution time: {:.2f} seconds",
              test_duration.count());
  SPDLOG_INFO(
      "============== End-to-End CPU+GPU Graph Construction Test Completed "
      "==============");
}
