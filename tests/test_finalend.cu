...
//   SPDLOG_INFO("Starting End-to-End CPU-GPU T2I 10M test");
...
//   SPDLOG_INFO("Base data loaded in {:.2f} seconds",
//   base_load_duration.count());
...
//     std::chrono::duration<double> matchnn_duration = matchnn_end -
//     match_start; SPDLOG_INFO(
...
//     SPDLOG_INFO("CPU thread: Top2 projection completed in {:.2f} seconds",
//                 top2_duration.count());
...
//     SPDLOG_INFO("CPU thread: Top3 projection completed in {:.2f} seconds",
//                 top3_duration.count());
...
//                  const_cast<float*>(h_base_data.data()), dim,
//                  cpu_thread_limit);
//     auto top4_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> top4_duration = top4_end - top3_end;
//     SPDLOG_INFO("CPU thread: Top4 projection completed in {:.2f} seconds",
//                 top4_duration.count());
...
//     std::chrono::duration<double> top5_duration = top5_end - top4_end;
//     SPDLOG_INFO("CPU thread: Top5 projection completed in {:.2f} seconds",
//                 top5_duration.count());
...
//                  const_cast<float*>(h_base_data.data()), dim,
//                  cpu_thread_limit);
//     auto top6_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> top6_duration = top6_end - top5_end;
//     SPDLOG_INFO("CPU thread: Top6 projection completed in {:.2f} seconds",
//                 top6_duration.count());
...
//     std::chrono::duration<double> top7_duration = top7_end - top6_end;
//     SPDLOG_INFO("CPU thread: Top7 projection completed in {:.2f} seconds",
//                 top7_duration.count());
...
//                  const_cast<float*>(h_base_data.data()), dim,
//                  cpu_thread_limit);
//     auto top8_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> top8_duration = top8_end - top7_end;
//     SPDLOG_INFO("CPU thread: Top8 projection completed in {:.2f} seconds",
//                 top8_duration.count());
...
//     std::chrono::duration<double> top9_duration = top9_end - top8_end;
//     SPDLOG_INFO("CPU thread: Top9 projection completed in {:.2f} seconds",
//                 top9_duration.count());
...
//                  const_cast<float*>(h_base_data.data()), dim,
//                  cpu_thread_limit);
//     auto top10_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> top10_duration = top10_end - top9_end;
//     SPDLOG_INFO("CPU thread: Top10 projection completed in {:.2f} seconds",
//                 top10_duration.count());
...
//     std::chrono::duration<double> top11_duration = top11_end - top10_end;
//     SPDLOG_INFO("CPU thread: Top11 projection completed in {:.2f} seconds",
//                 top11_duration.count());
...
//     SPDLOG_INFO(
//         "CPU thread: All Match Supplementary computations completed in {:.2f}
//         " "seconds", match_sup_duration.count());
...
//     SPDLOG_INFO("CPU thread: All CPU computations completed in {:.2f}
//     seconds",
//                 match_duration.count());
...
//   std::chrono::duration<double> cleanup_duration = cleanup_end -
//   cleanup_start; SPDLOG_INFO("Resource cleanup completed in {:.2f} seconds",
...
//   // 获取环境变量来控制供应图轮数和保存路径
//   int match_rounds = 6;  // 默认值：6轮匹配
//   std::string save_path_base =
//       "/home/yuxiang/GPUproject/indices/laion-1M/laion-gpu-std-r";
...
//   auto test_start_time = std::chrono::high_resolution_clock::now();
//   SPDLOG_INFO("Starting End-to-End CPU-GPU Laion 1M test with {} match rounds",
//               match_rounds);
...
//   cudaDeviceReset();
//   auto cuda_reset_time = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> cuda_reset_duration = cuda_reset_time -
//   test_start_time; SPDLOG_INFO("CUDA device reset completed in {:.2f}
//   seconds", cuda_reset_duration.count());
...
//   auto param_setup_time = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> param_setup_duration = param_setup_time -
//   cuda_reset_time; SPDLOG_INFO("Parameter setup completed in {:.2f} seconds",
//   param_setup_duration.count());
...
//   read_vec_from_file(h_gt_data, gt_file);
//   auto gt_load_time = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> gt_load_duration = gt_load_time -
//   data_load_start; SPDLOG_INFO("GT data loaded in {:.2f} seconds",
//   gt_load_duration.count());

//   read_vec_from_file(h_base_data, basedata_file_name);
//   auto base_load_time = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> base_load_duration = base_load_time -
//   gt_load_time; SPDLOG_INFO("Base data loaded in {:.2f} seconds",
//   base_load_duration.count());
...
//   auto stream_create_end = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> stream_create_duration = stream_create_end -
//   stream_create_start; SPDLOG_INFO("CUDA streams and events created in {:.2f}
//   seconds", stream_create_duration.count());
...
//   auto data_transfer_end = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> data_transfer_duration = data_transfer_end -
//   data_transfer_start; SPDLOG_INFO("Data transfer to GPU initiated in {:.2f}
//   seconds", data_transfer_duration.count());

//   // 3. CPU启动单独线程执行MatchNN（与GPU并行）
//   auto cpu_thread_start = std::chrono::high_resolution_clock::now();
//   SPDLOG_INFO("Starting CPU thread for MatchNN (will run in parallel with all
//   GPU work)");
...
//     auto matchnn_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> matchnn_duration = matchnn_end -
//     match_start; SPDLOG_INFO("CPU thread: MatchNN base computation completed
//     in {:.2f} seconds", matchnn_duration.count());
...
//     auto match_sup_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> match_sup_duration = match_sup_end -
//     match_sup_start; SPDLOG_INFO("CPU thread: All Match Supplementary
//     computations completed in {:.2f} seconds", match_sup_duration.count());

//     // 将前6个图添加到supply_graphs中
//     supply_graphs.push_back(top2_projection_graph);
//     supply_graphs.push_back(top3_projection_graph);
//     supply_graphs.push_back(top4_projection_graph);
//     supply_graphs.push_back(top5_projection_graph);
...
//     auto fusion_nn_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> fusion_nn_duration = fusion_nn_end -
//     fusion_nn_start; SPDLOG_INFO("CPU thread: FusionNN completed in {:.2f}
//     seconds", fusion_nn_duration.count());
...
//   auto gpu_phase1_end = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> gpu_phase1_duration = gpu_phase1_end -
//   gpu_phase1_start; SPDLOG_INFO("GPU Phase 1 launched in {:.2f} seconds",
//   gpu_phase1_duration.count());
...
//   auto gpu_phase2_end = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> gpu_phase2_duration = gpu_phase2_end -
//   gpu_phase2_start; SPDLOG_INFO("GPU Phase 2 launched in {:.2f} seconds",
//   gpu_phase2_duration.count());
...
//     cpu_worker.join();
//     auto cpu_thread_end = std::chrono::high_resolution_clock::now();
//     std::chrono::duration<double> cpu_thread_duration = cpu_thread_end -
//     cpu_thread_start; SPDLOG_INFO("CPU worker thread joined after {:.2f}
//     seconds", cpu_thread_duration.count());
//   }
...
//   auto stat_final_end = std::chrono::high_resolution_clock::now();
//   std::chrono::duration<double> stat_final_duration = stat_final_end -
//   stat_final_start; SPDLOG_INFO("Final graph statistics calculated in {:.2f}
//   seconds", stat_final_duration.count());
