...
TEST(GpuConstructionTime, TestEnd2EndCPUGPU) {
  ...
  std::thread cpu_worker([&]() {
    ...
    std::vector<std::vector<uint32_t>> top19_projection_graph =
        MatchSup(base_num, query_num, 19, gt_degree, 40,
                 const_cast<uint32_t*>(h_gt_data.data()),
                 const_cast<float*>(h_base_data.data()), dim, cpu_thread_limit);
    auto top19_end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> top19_duration = top19_end - top17_end;
    SPDLOG_INFO("CPU thread: Top19 projection completed in {:.2f} seconds", top19_duration.count());

    std::vector<std::vector<uint32_t>> top23_projection_graph =
        MatchSup(base_num, query_num, 23, gt_degree, 40,
                 const_cast<uint32_t*>(h_gt_data.data()),
                 const_cast<float*>(h_base_data.data()), dim, cpu_thread_limit);
    auto top23_end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> top23_duration = top23_end - top19_end;
    SPDLOG_INFO("CPU thread: Top23 projection completed in {:.2f} seconds", top23_duration.count());
    ...
    supply_graphs.push_back(top19_projection_graph);
    supply_graphs.push_back(top23_projection_graph);
  ...
  // KERNEL 9: fusion_merge_sort_prune_kernel_round2
  ...
  for (int i = 0; i < NUM_KERNELS; i++) {
    ...
    SPDLOG_INFO("  - {}: {:.3f} seconds", kernel_names[i], kernel_times[i]/1000);
  }
...
TEST(GpuConstructionTime, DISABLED_TestEnd2EndCPUGPU1) {
  auto test_start_time = std::chrono::high_resolution_clock::now();
  SPDLOG_INFO("Starting End-to-End CPU-GPU test");
...
TEST(GpuConstructionTime, DISABLED_TestEnd2EndCPUGPU0) {
  auto test_start_time = std::chrono::high_resolution_clock::now();
