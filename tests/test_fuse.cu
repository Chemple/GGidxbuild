...
TEST(GpuConstructionTime, TestEnd2EndCPUGPU_Laion_1M_512) {
  ...
  cudaMalloc(&d_distances, base_num * match_degree * sizeof(float));
  ...
  //         (pr_neighbor_list<uint32_t, reverse_edge_num, pruned_edge_num>*)
  //             d_space_128_xx);
  fuck_sort_by_distance<grid_size, block_size, base_num, match_degree,
                        pruned_edge_num, reverse_edge_num, tomb, dim, false,
                        float, uint32_t>
      <<<grid_size, block_size, 0, compute_stream>>>(
          d_base_data, d_space_128_yy, d_distances);
  fuck_prune<grid_size, block_size, base_num, match_degree, pruned_edge_num,
             reverse_edge_num, tomb, dim, false, float, uint32_t>
      <<<grid_size, block_size, 0, compute_stream>>>(
          d_base_data, d_space_128_yy, d_distances,
          (pr_neighbor_list<uint32_t, reverse_edge_num, pruned_edge_num>*)
              d_space_128_xx);
  fuck_add_reverse<grid_size, block_size, base_num, match_degree,
                   pruned_edge_num, reverse_edge_num, tomb, dim, false, float,
                   uint32_t><<<grid_size, block_size, 0, compute_stream>>>(
      d_base_data, d_space_128_yy, d_distances,
      (pr_neighbor_list<uint32_t, reverse_edge_num, pruned_edge_num>*)
          d_space_128_xx);
